...
系统为**单页应用**（`frontend/index.html`）：顶部 4 个功能导航（数据探索 / 特征工程 / 模型训练 / 预测分析）同页切换，二级导航区分**表格数据 / 图片数据**，左侧为数据集管理与"快撩我"聊天，无需页面跳转。

| 模块 | 说明 |
|---|---|
| 💬 快撩我 | 左侧常驻大模型聊天框（流式输出），模型先回复"你好呀，我是奶龙~ 快撩我啊"，分析中提问回复"等一下奶龙~"；可清空聊天 |
| 🔍 数据探索 | **数据概览与探索**：一键数据质量报告（数据形状 / 列名与类型 / 数值与文本统计摘要 / 缺失与重复）。**EDA 按需绘图**：图表类型下拉 + 特征列多选，大模型先分析意图再由后端 matplotlib 确定性绘制（直方图/折线图/柱状图/散点图/箱线图/小提琴图/相关性热力图等，支持多变量），图像**实时展现绘制过程**、可打包下载。**EDA 分析建议**为大模型问答（流式输出），基于 pandas 计算的**相关性/特征重要性**给出分析报告（折叠默认收起，可下载 `原数据集名_特征分析报告.md`）；绘图失败打印日志 |
| ⚙️ 特征工程 | 独立功能面板，全部后端确定性执行：**特征构造**（四大类：按分组列的聚合统计特征、两数值列组合交叉特征、从时间列提取年/月/日/星期/季度等时序特征、类别值频率与计数特征）；**数据变换与规范化**（standardize 标准化 / minmax 归一化 / robust 稳健缩放，替换所选数值列原值）；**智能预处理流水线**（诊断—治疗闭环：初步EDA → 预处理A 对数变换 log1p → 再次EDA 验证偏度/离群 → 效果不佳转预处理B Box-Cox 或分位数分箱 → 最终建模数据集，产出 `原数据集名_preprocessed`，数值列中位数填充）；**数据集切分**（训练/验证/测试三集，可分别下载） |
...
│   │   ├── clean/__init__.py      # 数据清洗与特征工程：意图解析 + 白名单确定性执行 / 智能清洗工作台 / 特征构造 / 数据变换
│   │   ├── eda/__init__.py        # EDA：数据概览与探索 / 选项式按需绘图（动画帧）/ 特征报告 / 打包下载
│   │   ├── preprocess/__init__.py # 智能预处理：初步EDA → 预处理A/B（log1p/Box-Cox/分箱）→ 再次EDA → 最终建模数据集
...
│       └── imgs/                  # 预测图表
├── frontend/                      # 前端（单页静态）
│   ├── index.html                 # 唯一入口：4 功能导航同页切换 + 表格/图片数据二级导航 + 快撩我 + 数据集管理
│   └── mediapipe/                 # 手势识别本地资源（ESM 库 / wasm 引擎 / 手势模型，离线可用）
...
浏览器自动打开 `http://127.0.0.1:8081/index.html`，直接进入系统。
...
### 数据清洗与特征工程（routes/clean/__init__.py）
| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/clean_dataset` | 清洗主接口：大模型解析意图 → pandas 白名单确定性执行（含失败日志） |
| POST | `/clean_run` | 智能清洗工作台：类型纠正 + 缺失值处理（删除/常数/统计量/插值）+ 重复值处理，返回清洗前后质量对比 |
| POST | `/clean_fast` | 快捷清洗：直接传白名单操作 JSON（不调用大模型） |
| POST | `/fe_construct` | 特征构造：聚合统计 / 组合交叉 / 时序 / 频率计数 四大类，后端白名单确定性执行 |
| POST | `/fe_transform` | 数据变换与规范化：standardize 标准化 / minmax 归一化 / robust 稳健缩放，替换所选数值列原值 |
...
| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/data_overview` | 一键数据质量报告：数据形状 / 列名与类型 / 数值与文本统计摘要 / 缺失与重复 |
...
### 智能预处理（routes/preprocess/__init__.py）
| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/preprocess_run` | 智能预处理流水线：初步EDA（偏度/离群诊断）→ 预处理A 对数变换 log1p → 再次EDA 验证 → 预处理B Box-Cox 或分位数分箱 → 最终建模数据集 `原数据集名_preprocessed`；数值列中位数填充，返回逐列变换方案与日志 |
...
- **模型不执行任意代码**：清洗操作走白名单（`CLEAN_OPS`）后端确定性执行；EDA 绘图由规则解析映射到固定绘图函数；特征构造 / 数据变换 / 智能预处理全部由后端确定性代码执行（前端只传选项，不拼接代码）
...
- 文件名统一格式：清洗 `原数据集名_clean`、智能预处理 `原数据集名_preprocessed`、特征报告 `原数据集名_特征分析报告`、三集 `原数据集名_train/_test/_val`、模型 `{算法}_best_loss/_last_loss`、图片评估报告 `数据集名_模型评估报告`
- 后端代码按功能分文件夹组织（`routes/chat`、`routes/image`、`routes/clean`、`routes/eda`、`routes/preprocess`、`routes/train`、`routes/predict`），新增功能请放入对应包并在 `main.py` 显式 import
